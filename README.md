...
- **机器人命令**：`enable_bot_commands` 开启后通过 WebSocket 实时接收命令——`/运势` 查询今日运势（同一天同一人结果固定，本地生成无外部依赖），`/抽人 [频道ID或#n]` 从指定频道在线成员中随机抽一位（支持频道简易 ID，留空则从所有监控频道抽取，需先在[机器人开发平台](https://bot.xiaoheihe.cn)注册同名命令）；`/init` 初始化频道简易 ID 映射（管理员专用，给在线语音频道分配 #1、#2 等简短 ID）；`/forcepushepic` 管理员强制推送一次 Epic 限免通知（跳过去重检查）；管理员权限自动从房间成员频道管理员角色读取，支持配置 `admin_user_ids` 兜底
- **Epic 限免通知**：设置 `epic_notify_channel_id` 后，每 30 分钟检查一次 Epic 免费游戏（数据来自 uapis.cn），有新限免时推送到指定频道，按游戏 ID 去重、重启不重复通知；管理员可用 `/forcepushepic` 强制推送一次
...
| `enable_bot_commands` | 是否启用机器人命令（`/运势`、`/抽人`、`/forcepushepic`），默认 false |
| `epic_notify_channel_id` | Epic 限免通知的频道 ID（可选，留空则不通知） |
| `admin_user_ids` | 管理员用户 ID 列表（用于需要管理员权限的命令，如 `/forcepushepic`） |
