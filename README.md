...
- 推送内容包括即将结束的游戏、当前可领游戏、即将免费的游戏、领取截止时间与商店链接（平台只允许内嵌黑盒 CDN 图片，故不带封面图）
- 游戏明细的类型、联机支持与媒体评分来自 [egdata.app](https://docs.egdata.app) 开放接口（无需鉴权），按 Epic offer ID 关联；评分按 offer ID 缓存 7 天，接口没有评分数据时对应行自动省略
- 补充信息抓取失败只影响这几行明细，不会阻塞推送
- 领取截止不足 24 小时的游戏会单独置顶到「🚨 即将结束」分组，加大提醒力度
- 支持配置多个推送时间点（默认每天 00:00，紧跟 Epic 固定 23:00 的轮换拿到最新列表）
...
| `epic_push_times` | Epic 推送时间点列表，格式 `["HH:MM"]`，默认 `["00:00"]` |
